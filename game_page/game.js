...
Ok, I'm not gonna spoil it, but once again, a very scary game!`
    ...
    ratingConvert : function (game_rating){
        switch (game_rating) {
            case 0:
                return `
    ...
    render : function (){
        ...
        var totalRating = 0;
        var countRating = 0;;
        getGamebyId.comments.forEach((comment)=>{
            totalRating += comment.comment_rating;
            countRating ++;
        })


        var gameRating = this.ratingConvert(Math.round(totalRating*2/countRating)/2);

                
        $(".product-container").innerHTML = `
        <div class="product-information-header">
            <div class="product-image" >
                <img src="../assets/image/game_thumbnail/${getGamebyId.image_thumbnail}" alt="${getGamebyId.image_thumbnail}">
            </div>
        ...
        <div class="product-description">
            <p class="product-description-1">${getGamebyId.description_1}</p>
            <div class="product-description-image">
                <img src="../assets/image/game_description/${getGamebyId.image_description}" alt="">
            </div>
            <p class="product-description-2">${getGamebyId.description_2}</p>
        </div>`;

        var commentHTMLs = getGamebyId.comments.map((comment,index)=>{
            var commentRating = app.ratingConvert(Math.round(comment.comment_rating*2)/2);
            return `
            <div class="product-comment">
                            <div class="comment-avt">
                                <img src="../assets/image/users_avatar/${comment.comment_avt}" alt="${comment.comment_avt}">
                            </div>
                            <div class="comment-container">
                                <h3 class="comment-name">${comment.comment_name}</h3>
        ...
        $(".comment-container").innerHTML = commentHTMLs.join("");
